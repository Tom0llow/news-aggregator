...
- 「記事を探す」のカテゴリ欄で保存済み記事のカテゴリを選び、完全一致の一覧へ絞り込めます。
  記事カード下部のカテゴリボタンも同じ絞り込みへの近道です。最新順または閲覧数順に
  切り替えられ、同じ閲覧数の記事は最新順で安定して表示します。
...
`GET /api/categories` は、保存済みの全記事に存在する空でないカテゴリを重複なしの昇順で返します。
